...
public class OVRDebugInfo : MonoBehaviour
{
    ...
    private bool _shouldShowDebugInfo = false;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && riftPresentTimeout < 0.0f || _shouldShowDebugInfo)
        {
            initUIComponent = true;
            showVRVars ^= true;
            _shouldShowDebugInfo = false;
        }
    ...
    void OnEnable()
    {
        EventManager.OnShowDebugInfo += ShowDebugInfo;
    }
